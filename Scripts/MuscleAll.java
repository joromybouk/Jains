...
public class MuscleAll {
	...
	private static void WriteJsonFile() throws IOException{
		...
		try {
			jsonWriter = new JsonWriter(new FileWriter("testAll.json"));
			...
			jsonWriter.beginObject();
   			...
   			for(int i = 0; i < muscleNames.size(); i ++){
	   			//jsonWriter.beginObject();
	   			//jsonWriter.name(muscleNames.get(i));
	   			//jsonWriter.beginArray();
	...
	private static void MakeJsonFilePretty() throws IOException{
		File jsonFile = new File("testAll.json");
		...
		FileWriter f = new FileWriter("testAll.json");
